...
given code snippet needs to be in a \code{try-catch} block or not.
...
each corresponding statement with one of three tags: \code{O} tag
means that the statement is outside any try-blocks; \code{B-Try} tag
means the statement begins a try-block; and \code{I-Try} tag means the
