...
statement in the given code snippet needs to be in a \code{try}
...
facilitates {\tool} to learn the relations of an API and its exception,
thus, making the connection between the API and the presence of
\code{try-catch} block.  For example, \code{write} in the statement 1
could be determined as having a relation with \code{IOException},
%belong to \code{OutputStream} in JDK. Thus,
...
label each corresponding statement with one of three tags: {\em
  ...
  statement is inside a \code{try-catch} block and is not the first
  line of that block}. For example, in Figure~\ref{fig:xstate}, the
embedding computed by CodeBERT for the first {\bf [SEP]}, which
...
because it is the first statement of a \code{try-catch}
block. However, the embedding for the second {\bf [SEP]}, representing
...
\code{try-catch} block. With this IOB2 encoding~\cite{iob2}, we can
support multiple \code{try-catch} blocks in which the statement with
the \code{B-Try} tag is the start of a \code{try-catch} block and the
