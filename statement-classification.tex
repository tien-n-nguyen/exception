\section{{\xstate}: \code{Try-Catch} Statement Detector}
\label{sec:xstate}

\begin{figure}[t]
 	\centering
 	\includegraphics[width=3.2in]{xstate-2.png}
        \vspace{-12pt}
...
statement in the given code snippet needs to be in a \code{try-catch}
...
\code{try-catch} blocks.  For example, \code{write} in the statement 1
could be determined to belong to \code{OutputStream} in JDK. Thus,
that leads to better learning to place that statement inside a
\code{try-catch} block. Second, we expect that CodeBERT would learn
     {\em the dependencies among the consecutive statements} separated
     by the special tokens {\bf [SEP]} (see Section~\ref{sec:rq4} for our
     experiment on this). Those dependencies would help the model better
     decide whether some consecutive statements need to be placed
     together in a \code{try-catch} block. For example, in
     Figure~\ref{overview}, the statements at lines 2--5 have data
     dependencies, thus, they should be in the same \code{try-catch}
     block.
...
{\em \code{O} tag means that the statement is outside of any
\code{try-catch} blocks; \code{B-Try} tag means that the statement
begins a \code{try-catch} block; and \code{I-Try} tag means that the
statement is inside a \code{try-catch} block and is not the first line
...
statement \code{out.write(...)}, is classified by the first
\code{FFNN+Softmax} layer into the \code{B-Try} category/label because
it is the first statement of a \code{try-catch} block. However, the
embedding for the second {\bf [SEP]}, representing
\code{print(node.getBody(...))}, is labeled as \code{I-Try} because it
is the second statement of the \code{try-catch} block. Finally, the
embedding of the third {\bf [SEP]}, is labeled as \code{O} because the
statement \code{return null;} does not need to be in the
\code{try-catch} block. With this tag encoding, we can support
multiple \code{try-catch} blocks in which the statement with the
\code{B-Try} tag is the start of a \code{try-catch} block and the
...
statements, from which we can derive the \code{try-catch} blocks and
