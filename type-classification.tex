...
\code{try-catch} block and the corresponding exception types that need
...
snippet. For each \code{try-catch} block, we identify the statements
at the beginning (with the \code{B-Try} label) and at the end of a
\code{try-catch} block (with the last respective \code{I-Try} label).
...
of the \code{try-catch} block. We add them together to get the
embedding for the entire \code{try-catch} block, and then feed it into
...
statements in a predicted \code{try-catch} block. From there, the
...
predicts the \code{try-catch} block from the statement
\code{out.write} to the statement \code{print(node.getBody(...))}. The
corresponding embeddings of all the statements in the block are used
and the output of \code{IOException} is predicted with the highest
probability.
