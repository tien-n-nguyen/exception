\subsubsection{{\xstate}'s Effectiveness on Try-block Statement Detection (RQ2)}
...
multiple) \code{try} blocks in the oracle. To do so, we match the
encoded vector \code{[O,B-Try,I-Try]} of the prediction against the
...
a \code{try} block, regardless of the blocks themselves. Thus, we use
...
a \code{try-catch} block as well as on the statement tagging). Second,
