...
\subsection{Comparison on \code{Try-catch} Block Checking}
%\label{sec:rq1}
...
baseline. First, XRank's recall is around 0.53 in our balanced dataset. In
...
%
Unlike XRank, which considers only the aforementioned association
between an API call and an exception type,
{\tool} considers the code in the block as the context to learn the
dependencies among the API elements and the relations between the API
...
\code{try-catch} block because that API requires such a block. For
...
compiler, which needs to handle \code{InvalidInputException}. Thus, it
incorrectly considers \code{parseVals} needs to handle that exception.
