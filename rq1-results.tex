...
Precision, tantamount to a random classifier.  However, it achieves a
higher Recall. After examining the result, we find that the model
...
30,764 samples, only 236 samples receives the negative label from 
CodeBert.
...
In addition, we partition the test dataset according to the number of try-catch blocks, and evaluate {\xblock} on each partition. As can be seen in Table~\ref{tab:rq1-detailed-result}, {\xblock}
gives 100\% correct prediction on the partitions with zero, three and five try-catch blocks. Furthermore, it achieves 100\% precision and above 0.99 F1 score across all the partitions, which shows
that the prediction ability of {\xblock} remains strong regardless of the number of try-catch blocks in a code snippet.
