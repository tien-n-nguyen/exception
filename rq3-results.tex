...
{\tool} covers on the actual correct exception types in \code{catch}
...
types in \code{catch} clauses: $K$ = the number of Exception Types in
a \code{catch} clause = $1,2,3+$. For example, in the oracle, there
are 547 instances with 3 exception types in a \code{catch}
clause. {\tool}'s predicted set correctly contains all 3 exception
types for 115 instances (21\%) (\code{Hit-All}), contains 2 of them
...
Note that \code{Hit-All} = \code{Hit-$N$} when $N$(\# overlaps) = $K$
(\# exception types). For $K$=1..3, which is a total of 43,557
instances (99.1\%), {\tool} covers all the exception types {\bf
  (\code{Hit-All}) in 27,037 instances (62\%)}. That is, developers do
