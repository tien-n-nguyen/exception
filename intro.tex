...
2) the trained GCN model along with its decision on whether a
\code{try-catch} block is needed for $C$.
%
GNNExplainer will learn the output explainable subgraph, which is
...
improves over the state-of-the-art IR approach in exception type
...
handling recommendation for exception blocks, statements, and types.
