...
code that are potentially involved in the exceptions in a \code{catch}
clause. XHand~\cite{xrank-fse20} recommends the exception handling
code in a \code{catch} block for a given code. Both use a
fuzzy set technique to compute the associations between the API calls
(e.g., \code{new\-Buffered\-Reader}) and the exceptions (e.g.,
\code{IOException}).
...
\code{try-catch} blocks and the exception types retrieved from complete
...
\code{try-catch} block as a classification (Y/N) as well as the
...
\code{try-catch} block being needed for $C$.
%
...
to be placed in a \code{try-catch}~block.

We have conducted several experiments to evaluate {\tool}. We have
collected a large dataset of 5,726 projects from Github, with 19,379 code snippets that contains one \code{try-catch} blocks.
%
