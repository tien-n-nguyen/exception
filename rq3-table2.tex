...
\code{IllegalArgumentException}, \code{IOException},
\code{IndexOutOfBoundException}, \code{RuntimeException}, etc. For
unpopular API calls, it resorted to using the general exception
\code{Exception} or \code{APIException} as an answer. For the cases of
multiple \code{try-catch} blocks, the common errors are the use of
\code{Exception} for all blocks.
