	...
	\begin{center}
		...
		Instance Level	  & Precision  & Recall & F1-score \\
			\hline
                        ...
                        GPT-3 & {\bf 0.XX} & {\bf 0.YY} & {\bf 0.ZZ} \\
			\hline
			\xblock + \xstate   & \textbf{0.9688}  &  \textbf{0.6072} & \textbf{0.7465}\\
			\hline
		\end{tabular}
		\label{tab:xstate-1}
	\end{center}
...
improves relatively over GPT-3 {\bf XX.X\%}, {\bf YY.Y\%}, and {\bf
  ZZ.Z\%} in Precision, Recall, and F1-score. Examining the results,
we found that GPT-3 did not work well for the \code{try-catch} blocks
with several statements in which some statements are left outside of
the blocks.
...
or not). As seen, {\xblock}+{\xstate} improves relatively over GPT-3
