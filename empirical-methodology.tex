...
statements within \code{try-catch} blocks are used for positive
...
\code{Hit-All}$_{Rec}$ as \code{Hit}-$n$ when the number $n$ of
...
define \code{Hit-All}$_{Prec}$ as \code{Hit}-$n$ when the number $n$
...
\code{try-catch} blocks and/or exceptions. We used {\tool} to detect
...
\code{try-catch} block and/or exceptions), we consider it as correct.
