...
Figure~\ref{fig:xblock} shows the illustration of {\xblock}.
...
into sub-tokens using CodeBERT's tokenizer. We use a special
separator token \texttt{[SEP]} to concatenate the tokenized
statements, and add a \texttt{[CLS]} token at the beginning. As in
CodeBERT, we take that \texttt{[CLS]} token to be the representation of
...
needs to be placed in a \code{try} block.
